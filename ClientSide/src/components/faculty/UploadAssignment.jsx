...
function UploadAssignment() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert('Assignment uploaded! (Demo)');
  };

  return (
    <div className="max-w-2xl mx-auto mt-10 bg-white p-8 rounded-lg shadow-md">
      <h2 className="text-2xl font-bold text-purple-800 mb-4">Upload Assignment</h2>
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <input type="text" value={title} onChange={e => setTitle(e.target.value)} className="w-full border rounded px-2 py-1" placeholder="Assignment Title" />
        </div>
        <div className="mb-4">
          <label className="block text-gray-700 mb-1">Description</label>
          <textarea value={desc} onChange={e => setDesc(e.target.value)} className="w-full border rounded px-2 py-1" placeholder="Assignment Description" />
        </div>
        <div className="mb-4">
          <label className="block text-gray-700 mb-1">Due Date</label>
          <input type="date" value={due} onChange={e => setDue(e.target.value)} className="w-full border rounded px-2 py-1" />
        </div>
        <div className="mb-6">
          <label className="block text-gray-700 mb-1">File</label>
          <input type="file" onChange={e => setFile(e.target.files[0])} className="w-full" />
        </div>
        <button type="submit" className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700">Upload Assignment</button>
      </form>
    </div>
  );
}
